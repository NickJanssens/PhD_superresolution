import numpy as np
from keras.layers import Conv2D, BatchNormalization, MaxPooling2D, UpSampling2D, Conv2DTranspose
...
def generator_unet_real(input_shape):
    ...
    def deconv2d(layer_input, skip_input, filters, f_size=3, dropout_rate=0):
        """Layers used during upsampling"""
        u = Conv2DTranspose(filters, kernel_size=f_size, strides=2, padding='same',
                   kernel_initializer=init)(layer_input)
        u = LeakyReLU(alpha=0.2)(u)
...
def discriminator(input_shape):
    ...
    d5 = Conv2D(1, kernel_size=4, strides=1, padding='same', kernel_initializer=init)(d4)
    d6 = Flatten()(d5)
